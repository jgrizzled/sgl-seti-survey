...
WISPR-I lambda_c 615 nm, W_eff 250 nm; a +-0.3 mag band-conversion
systematic is declared). S1: P_cone = F_line * pi * (0.1 AU)^2 (the beam at the
...
import wispr_lib as H
import series as S

REPO = Path(__file__).resolve().parents[3]
SURV = REPO / "surveys" / "wispr-crossings"
SER = REPO / "runs" / "wispr-crossings" / "series"
...
LAMBDA_C, W_EFF = 615e-9, 250e-9   # WISPR-I 490-740 nm
...
def stats_from(series: dict, inject: float = 0.0, ratios=None, rng=None,
    ...
    for i, (key, se) in enumerate(series.items()):
        ...
        if pulse is not None and pulse[0] == i:
            r = float(rng.choice(ratios)) if ratios is not None and len(ratios) else 1.0
            d = d.copy()
            d[pulse[1]] += pulse[2] * r            # v1.5 pulse cell: two consecutive frames
            if pulse[1] + 1 < len(d):
        ...
        x = (d - np.median(d)) / sd
        pulses.append(float(np.max(np.minimum(x[:-1], x[1:]))))   # v1.5 pair-min
...
def main(role: str = "confirmatory", version: str = "v1") -> None:
    ...
    out = {"stage": f"completeness_{version}", "role": role, "seed": S.SEED,
           ...
           "band": {"lambda_c_nm": 615, "w_eff_nm": 250, "dnu_Hz": DNU,
                    "systematic_mag": 0.3, "note": "V_eq of a B-V 0.65 source; 532 nm line in band; 1064/1550 nm as broadband leakage"},
           "flux_scale_systematic_mag": 0.3,
           "flux_scale_note": "the ZP surface reads V 3-6 calibrators 0.23-0.33 mag faint (noise-boosted faint calibrators); every V_eq/power limit carries +-0.3 mag (dev_v1.md); power limits are quoted at the nominal scale",
           "pulse_cell": "two consecutive frames (v1.5 pair-min): injections span two epochs",
           "units": results}
