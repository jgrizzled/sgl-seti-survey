...
# check to exist. Usage: nohup bash surveys/panstarrs-v2/scripts/run_batches.sh > runs/panstarrs-v2/batches.log 2>&1 &
set -u -o pipefail
export OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1
cd "$(dirname "$0")/../../.."
S1=surveys/panstarrs/scripts
R=surveys/panstarrs-v2/run.py
BATCH=${BATCH:-4}
WORKERS=${WORKERS:-6}
ALL=$(uv run python -c "import sys; sys.path.insert(0,'surveys/panstarrs-v2'); from profile import PROFILE as P; print(' '.join(P.corridors()))")
set -- $ALL
