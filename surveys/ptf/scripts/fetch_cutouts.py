"""Fetch science-image cutouts for every usable/partial precise-pass
exposure (PTF serves no difference images; port of
surveys/ztf/scripts/fetch_cutouts.py). Cutout centre/size are taken
from the precise pass's cutout index so sci/msk cutouts share one
pixel grid. Writes runs/ptf/products/cut/manifest.jsonl.

Usage: uv run python surveys/ptf/scripts/fetch_cutouts.py [--workers N]
       [--corridors c1 c2 ...]
"""
...
PRECISE_DIR = REPO / "runs" / "ptf" / "precise_v1"
CUT_DIR = REPO / "runs" / "ptf" / "products" / "cut"
KINDS = ("sci",)
...
def main() -> None:
    ...
    ap.add_argument("--corridors", nargs="*", default=None,
                    help="restrict to these corridors (pilot stage)")
    args = ap.parse_args()
    ...
    todo = [o for o in usable if o not in done and o in cut_index
            and (only is None or usable[o] & only)]
    ...
    def work(oid):
        ...
        adapter = PtfLevel1Adapter(session=requests.Session())
        files, missing = {}, []
        for kind in KINDS:
