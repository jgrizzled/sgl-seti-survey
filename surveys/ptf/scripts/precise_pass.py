"""Precise intersection pass for the PTF/iPTF corridor survey (plan §4;
port of surveys/ztf/scripts/precise_pass.py).

Consumes coarse-stage hits from runs/ptf/coarse_v1, fetches a dmask
cutout per hit CCD exposure (centred on the union of the Rx and Tx
loci at mid-exposure, sized to enclose both), then evaluates sglseti's
covered_z_intervals against the exact TAN-SIP WCS + Laher (2014) dmask usable-pixel
footprint. Emits precise-stage IntersectionEvaluation records.

Usage:
    uv run python surveys/ptf/scripts/precise_pass.py [endpoint ...]
                  [--limit N]
"""
...
from sglsurvey.adapters.irsa_ptf import PtfExactFootprint, PtfLevel1Adapter
...
COARSE_DIR = REPO / "runs" / "ptf" / "coarse_v1"
RUN_DIR = REPO / "runs" / "ptf" / "precise_v1"
PRODUCT_DIR = REPO / "runs" / "ptf" / "products" / "msk"
REGISTRY_PATH = REPO / "registries" / "pilot_wise_2026.yaml"
HYPOTHESES_PATH = REPO / "surveys" / "ptf" / "hypotheses.md"
...
#: Locus spans <= ~375" (z=550 AU parallax) plus padding; 600 pix = 606"
#: at 1.01"/pix (PTF CCD is 2048 x 4096, so a cutout never exceeds it).
CUTOUT_PIX = 600
...
def main() -> None:
    ...
    print(f"{len(obs_ids)} hit CCD exposures, {n_evals} precise "
    ...
    run_config = {
        ...
        "fatal_mask_template": PtfExactFootprint.FATAL_MASK,
        "registry_source_hash": registry.source_hash,
        "hypothesis_version": HYPOTHESIS_VERSION,
        "hypothesis_hash": hyp_hash,
        "geometry": ctx.identities(),
        "interval_model": ("mid-exposure evaluation; 60 s frames move "
                           "the locus < 5 mas"),
    ...
    def fetch(oid, cutout):
        ...
        ad = PtfLevel1Adapter(session=_rq.Session())
    ...
    for c0 in range(0, len(obs_ids), CHUNK):
        ...
        for j, oid in enumerate(chunk):
            ...
            fp = (PtfExactFootprint(pset.products[0].path) if available
                  else None)
            infobits = int(obs.quality_flags.get("infobits") or 0)
            bad_quality = False  # PTF has no archival bad-quality bit; infobits recorded
            for (endpoint_id, role_name), al in loci.items():
                ...
                if fp is None:
                    ...
                    hit = len(covered) > 0
                    if bad_quality:
                        label, note = "unusable", "archival bad-quality flag"
                    elif not hit:
