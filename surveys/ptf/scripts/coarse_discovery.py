"""Coarse discovery pass for the PTF/iPTF corridor survey (plan §4;
coarse stage of Pipeline A). Port of surveys/ztf/scripts/coarse_discovery.py
onto the IRSA PTF level-1 adapter (same Palomar P48 observer).

For each endpoint x role: build a conservative discovery cone with
sglseti (Palomar observer), query the IRSA IBE PTF level-1 science-image
inventory (snapshotting the raw CSV), then evaluate the adaptive locus
at exposure-clustered epochs against each CCD exposure's nominal WCS.
Emits Observation and coarse-stage IntersectionEvaluation records
(hits AND misses).

Usage:
    uv run python surveys/ptf/scripts/coarse_discovery.py [endpoint ...]
Defaults to every ZTF-visible endpoint (PTF shares the site); records
land in runs/ptf/coarse_v1/.
"""
...
from ptf_corridors import ALL_ENDPOINTS  # noqa: E402

REPO = Path(__file__).resolve().parents[3]
RUN_ID = "coarse_v1"
RUN_DIR = REPO / "runs" / "ptf" / RUN_ID
...
# PTF first light (2009-03) through the last public iPTF level-1 epoch
# (2015-01), generous on both ends.
TIME_RANGE = MjdRange(start_mjd_utc=54850.0, stop_mjd_utc=57200.0)
...
def main(endpoint_ids: list[str]) -> None:
    ...
    run_config = {
        ...
        "archive": {"adapter": "PtfLevel1Adapter",
    ...
    for endpoint_id in endpoint_ids:
        ...
        n_bad = sum(int(o.quality_flags.get("infobits") or 0) != 0
                    for o in observations)
        print(f"[{endpoint_id}] discovered {len(observations)} CCD "
              f"exposures ({n_bad} with nonzero infobits) "
              f"({_time.monotonic() - t0:.0f}s)", flush=True)
                ...
                for o in cluster:
                    ...
                    ixn = IntersectionEvaluation.build(
                        ...
                        extra={"cluster_pad_arcsec": CLUSTER_PAD_ARCSEC,
                               ...
                               "infobits": int(
                                   o.quality_flags.get("infobits") or 0),
                               "photcalflag": int(
                                   o.quality_flags.get("photcalflag") or 0)},
                    )
