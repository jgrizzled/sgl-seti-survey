"""Stage-7 injection calibration and constraint generation for the ZTF
pilot. Same estimator as WISE calibration v0.2.0 (8-offset-control
thresholds, effective-epoch weight cap, duty-cycle-randomised analytic
injections exploiting matched-filter linearity, 90% recovery), with ZTF
specifics: per-epoch seeing sets the mismatch factors; depths quoted in
AB mag at the quadrant MAGZP scale; every threshold exceedance becomes a
Candidate adjudicated by the parallax-phase split test; each constraint
carries a reflected-light size conversion (asteroid H-D convention,
D_km = 1329 / sqrt(p) * 10^(-H/5), H = m - 5 log10(z^2), z in AU).

Usage: uv run python surveys/ztf/scripts/injection_calibrate.py
"""
...
CLIP_SIGMA = 5.0
#: Empirical throughput correction from the asteroid positive control
#: (runs/ztf/control_v1: (60000) recovered 0.41 mag (g) / 0.59 mag (r)
#: fainter than the Horizons prediction after solar colours g-V=+0.25,
#: V-r=+0.19). Attributed to the Gaussian-PSF model vs the real Moffat
#: profile; applied to every quoted depth in the conservative direction.
CONTROL_THROUGHPUT_MAG = 0.5
BAND_NAME = {1: "zg", 2: "zr", 3: "zi"}
...
def main() -> None:
    ...
    config = {
        "pipeline_id": "ztf-injection-calibration",
        ...
        "z_grid": "192 nodes uniform in 1/z, 550-10000 AU",
    ...
    for path in tensor_files:
        ...
        for b in sorted(set(band_idx.tolist())):
            ...
            dfb = np.asarray(d["dfrac"][:, eb], dtype=np.float32)
            with np.errstate(invalid="ignore"):
                dmean = (np.nan_to_num(dfb[0]) * w[0]).sum(axis=0) / np.where(
                    B[0] > 0, B[0], np.nan)
            ...
            threshold_report[key] = {
                ...
                "diff_frac_at_peak": float(dmean[i_real]),
            ...
            meta_store[key] = {"z_grid": z_grid,
                               ...
                               "dfrac_z": np.nanmean(dmean, axis=(1, 2)),
    ...
    for key, curves in m90_store.items():
        ...
        for k in range(N_Z_INTERVALS):
            ...
            if n_valid == 0:
                ...
                dfz = meta_store[key]["dfrac_z"][lo:hi]
                limit = {"value": mlim, "unit": "ztf_ab_mag", "band": band,
                         ...
                         "diff_image_weight_frac": round(
                             float(np.nanmean(dfz)), 2),
    ...
    for key in sorted(m90_store):
        ...
        if np.isfinite(m).any():
            print(f"  {key:22s} N={r['n_epochs']:3d} cells={r['cells_defined_frac']:.2f} "
                  ...
                  f"worst={np.nanmin(m):5.2f} dfrac@peak={r['diff_frac_at_peak']:.2f}")
