"""Shared corridor table for the universal target portfolio (registry
v1.5: 88 endpoints / 77 corridors).

One corridor per stellar system; multiple component endpoints share a
corridor (their arcs overlap at arcsecond scale). Promoted from
``surveys/wise/scripts/wise_corridors.py`` (v2 plan §8.5) so that no
survey directory depends on another; the per-survey ``*_corridors.py``
modules derive their membership from this table.
"""
